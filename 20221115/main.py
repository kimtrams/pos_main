...
def miner(blockchain):
    while STOP == 0:
        ...
        while (len(blockchain.blockqueue) >= 1):
            ...
            f.write(msg)
            f.close()
            #print("blockchain.blockqueue",blockchain.blockqueue[0].info.pos_miner)
            try:
                if (blockchain.blockqueue[0].info.pos_miner == (str(node.server_ip)+" "+str(node.server_port))):
                    ...
                    if count > 35:
                        if (blockchain.blockqueue[1].info.pos_miner == (str(node.server_ip)+" "+str(node.server_port))):
                            if (blockchain.blockqueue[1].timestamp < blockchain.blockqueue[0].timestamp):
                                _ = blockchain.blockqueue[1]
                                blockchain.blockqueue[1] = blockchain.blockqueue[0]
                                blockchain.blockqueue[0] = _
                    print("[problem_2_count]",count)
                    print("[problem_2]",blockchain.blockqueue[0].info.pos_miner)
            except:
                print("[problem]")
                pass
    pass
...
def new_block():
    ...
    print("[test_pos]",block.info.data)
    print("[test_pos_2]",block.info.pos_miner)
...
if __name__ == "__main__":
    ...
    while STOP == 0:
        ...
        if msg == "show blockqueue":
            for _ in range(len(blockchain.blockqueue)):
                print("[test_data]",blockchain.blockqueue[_].info.data)
                print("[test_miner]",blockchain.blockqueue[_].info.pos_miner)
        if msg == "show block":
